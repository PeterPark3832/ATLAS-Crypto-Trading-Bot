...
| `SPOT_MAX_ALLOC_PCT` | 14% | 단일 종목 최대 배분 |
| `SPOT_MIN_ALLOC_PCT` | 2% | 단일 종목 최소 배분 |
| `SPOT_MAX_POSITIONS` | 6 | 동시 보유 종목 상한 |
...
> **슬롯 수는 자본에도 연동됩니다**: `min(6, 자본 ÷ $20)`. 소액 계좌에서 과분할되면 주문이 NOTIONAL 턱걸이가 되고 수수료 드래그만 커지기 때문입니다.
>
> **배분 상한과 슬롯 수는 함께 봐야 합니다.** `6슬롯 × 14% + 예비금 10% = 94%`로 약 6%의 여유가 남습니다. 이 여유가 없으면 수수료 때문에 마지막 슬롯이 채워지지 않습니다 — 실제로 상한이 15%였을 때 `6 × 15% + 10% = 100%`가 되어 6번째 진입이 $0.43 차이로 차단됐고, **설정은 6인데 실제로는 5개만 채워졌습니다.** 둘 중 하나를 바꿀 때는 반드시 `슬롯수 × 배분상한 + 예비금 < 100%`를 확인하세요.
