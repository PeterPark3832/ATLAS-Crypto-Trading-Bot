...
#
# 상한이 걸리는 조건은 **SL거리 < 리스크 ÷ 배분상한 = 0.020/0.14 = 14.3%**
# 이다. 전형 SL은 5% 안팎이므로 **거의 모든 거래가 상한 쪽에서 결정**된다.
#
#   SL  2% → 실효 0.28%   ← 배분상한
#   SL  5% → 실효 0.70%   ← 배분상한 (전형 구간)
#   SL 10% → 실효 1.40%   ← 배분상한
#   SL 14.3% 이상 → 실효 2.00% (= 설정값. 여기서부터 상한이 안 걸린다)
#
...
#    슬롯 상한이 6이고 종목당 14%이므로, 6종목이 동시에 손절당하면
#    손실은 6 × 0.70%(전형 SL 5%에서의 실효) = **4.2%**다.
...
# 살아나는데, 그건 6종목 포트폴리오를 2~3종목으로 몰아넣는 것과 같고
# 애초에 가용자본(90%)을 넘어 6슬롯을 채울 수 없게 된다.
SPOT_MAX_POSITIONS     = 6       # 최대 동시 보유 종목 수 (15 → 6, 2026-08).
                                 # 자본이 지탱하는 종목 수는 **Kelly에 따라
                                 # 변한다**. 포지션 하나의 크기가
                                 #   min(자본×리스크×스케일/SL거리, 자본×15%)
                                 # 이므로, Kelly가 1.0이면 종목당 14%(=6개면
                                 # 가용자본의 93%를 쓴다), Kelly가 하한
...
SPOT_BASE_RISK_PCT     = 0.020   # 거래당 리스크 **상한**.
                                 # ⚠️ SL거리 14.3%(=0.020/0.14) 미만이면
                                 # 배분상한이 먼저 걸려 실효 리스크는
                                 # `배분상한 × SL거리`가 된다(전형 SL 5%에서
                                 # 0.70%). 위 주석 블록을 반드시 읽을 것.
SPOT_MAX_ALLOC_PCT     = 0.14    # 단일 종목 최대 배분 14% (0.15 → 0.14, 2026-08).
                                 # 백스톱이 아니라 **사실상의 주 사이징 축**이다
                                 # (전형 SL 구간에서 이쪽이 먼저 걸린다).
                                 # ⚠️ 15%였을 때 6슬롯 × 15% + 예비금 10% =
                                 # **정확히 100%**라 수수료가 들어갈 자리가 없었다.
                                 # 실측: 6번째 진입이 가용 $86.57 vs 주문 $87.00,
                                 # 즉 $0.43 차이로 차단돼 **설정은 6인데 실제로는
                                 # 5개만 채워졌다.** 14%면 6슬롯이 93% 를 쓰고
                                 # 약 $34 여유가 남아 6개가 확실히 채워진다.
                                 # 이 값을 바꿀 때는 6 × 값 + SPOT_RESERVE_PCT 가
                                 # 100%보다 충분히 작은지 반드시 확인할 것.
