...
# (SPOT_MAX_ALLOC_PCT). 둘의 관계를 이해하지 않으면 설정이 거짓말을 한다.
#
#   주문 명목가 = min( 자본 × 리스크 / SL거리 ,  자본 × 배분상한 )
#
# 즉 **SL거리 < 리스크/배분상한** 이면 상한이 걸리고, 그 순간부터
# SPOT_BASE_RISK_PCT는 아무 일도 하지 않는다.
#
# ⚠️ 2026-08 이전 설정(리스크 2.0% / 상한 15%)은 SL 15% 미만인 **모든**
#    거래에서 상한이 걸렸다. 그 결과:
#      · 설정은 2%인데 실효 리스크는 SL 5%에서 0.75%였다(설정이 죽어 있었다)
#      · 더 나쁜 건 리스크가 **SL 거리에 비례**했다는 점이다:
#          SL  2%(타이트=확신)  → 0.30%
#          SL 10%(넓음=불확실)  → 1.50%
#        불확실한 거래에 5배 크게 거는 셈으로, 리스크 기반 사이징이
#        존재하는 이유(SL 거리와 무관하게 리스크를 일정하게)의 정반대다.
#
# 그래서 리스크를 0.75%로 내려 **전형 SL(5%)에서 상한과 정확히 만나게**
# 했다. 이제 SL 5% 이상에서는 리스크가 0.75%로 일정하고, SL 5% 미만에서만
# 상한이 걸려 집중도 제한이 의도대로 backstop으로 동작한다.
# 전형 SL에서의 주문 크기는 이전과 **완전히 동일**하다(행동 불변).
#
# 참고: 상한이 걸리는 구간에서 **포트폴리오 전체 리스크는 상한 값과 무관**하다.
#   가용자본(90%) × SL거리 = 4.5% 로 고정이고, 상한은 "몇 종목으로 쪼갤지"만
#   정한다. 리스크를 키우고 싶다면 상한을 올려야 하는데, 그건 곧 집중이다.
# ── 값을 바꾸기 전에 반드시 읽을 것 ──────────────────────────────
# 리스크를 0.0075(= 상한 0.15 × 전형 SL 0.05)로 내리면 위 역전이 사라지고
# 설정이 진실을 말하게 된다. **다만 공짜가 아니다.**
#
#   배분 상한은 Kelly에 곱해지지 않는다. 그래서 Kelly가 1.0일 때는 상한이
#   걸려 주문 크기가 동일하지만, **연패로 Kelly가 내려간 구간**에서는
#   왼쪽 항이 상한 아래로 떨어져 base risk가 그대로 반영된다:
#
#       Kelly 1.00 →  $45.00 → $45.00  (동일)
#       Kelly 0.15 →  $18.00 →  $6.75  (−62%)
#
#   그 결과 "연패 중에도 전 조합이 멈추지 않는 최소 자본"이
#   **$556 → $1,481**로 오른다. 소액 계좌에서는 회복이 가장 필요한
#   순간에 거래가 더 일찍 끊긴다는 뜻이다.
#
# 판단 기준: 자본 $1,500 이상이면 0.0075로 내리는 편이 낫고, 그 아래면
# 리스크 역전을 감수하더라도 현행(0.020)이 낫다. 바꾸기 전에
# `python capital_plan.py --equity <자산>`로 문턱을 확인할 것.
SPOT_MAX_POSITIONS     = 15      # 설정상 최대 동시 포지션 수.
                                 # ⚠️ **실제 한계는 6개다** — 가용자본 90%를
                                 # 종목당 상한 15%로 나눈 값. 15개를 채우려면
                                 # 종목당 6%씩이어야 하는데 상한이 15%라
                                 # 그만큼 작게 잡히지 않는다. 이 값을 6으로
                                 # 낮춰도 동작은 같고, 표시만 정직해진다.
SPOT_BASE_RISK_PCT     = 0.020   # 거래당 기본 리스크.
                                 # ⚠️ 상한(0.15)이 걸려 **SL 15% 미만의 모든
                                 # 거래에서 이 값은 무효다.** 실효 리스크는
                                 # `배분상한 × SL거리`로 결정된다.
                                 # 기동 시 `_report_risk_profile`이 실제
                                 # 프로파일을 로그로 남긴다.
SPOT_MAX_ALLOC_PCT     = 0.15    # 단일 종목 최대 배분 15% (집중도 제한)
